...
import {
  ...
  FileText, Clock, DollarSign, UserCheck, Search,
  Trash2, CheckCircle,
} from 'lucide-react'
import { useAuth } from '../contexts/AuthContext'
import {
  fetchAllUsers, fetchNotesStats, updateUserPlan, grantFreeAccess, blockUser,
  reactivateUser, deleteUser, fetchNotesByType, fetchNotesBySpecialty, fetchNotesByDay,
} from '../lib/adminDb'
import type { UserSummary, NoteTypeCount, SpecialtyCount, DayCount } from '../lib/adminDb'
...
const NOTE_TYPE_LABELS: Record<string, string> = {
  ingreso:      'HC Ingreso',
  evolucion:    'Nota Evolución',
  telemedicina: 'Telemedicina',
  traslado:     'Ing. Traslado',
}
...
export default function AdminDashboard() {
  ...
  const [notesByType, setNotesByType] = useState<NoteTypeCount[]>([])
  const [notesBySpecialty, setNotesBySpecialty] = useState<SpecialtyCount[]>([])
  const [notesByDay, setNotesByDay] = useState<DayCount[]>([])
  ...
  useEffect(() => {
    ...
    Promise.all([fetchNotesByType(), fetchNotesBySpecialty(), fetchNotesByDay()]).then(([t, s, d]) => {
      setNotesByType(t)
      setNotesBySpecialty(s)
      setNotesByDay(d)
  ...
  const maxGrowthDay = Math.max(...growthData.map(d => d.count), 1)
  ...
  // ── Notes chart ───────────────────────────────────────────────────────────────
  const maxDayNotes       = Math.max(...notesByDay.map(d => d.count), 1)
  const maxSpecialtyCount = Math.max(...notesBySpecialty.map(s => s.count), 1)
  const maxTypeCount      = Math.max(...notesByType.map(t => t.count), 1)
  ...
  async function applyReactivate(userId: string) {
    ...
    setUsers(prev => prev.map(u => u.id === userId ? { ...u, subscription_status: 'active', consultations_limit: 250 } : u))
    setBusyId(null)
  }
                  ...
                  {growthData.map(({ label, count }) => (
                    <div key={label} className="flex-1 flex flex-col items-center gap-1">
                      ...
                      <div
                        ...
                        style={{ height: `${Math.max((count / maxGrowthDay) * 80, count > 0 ? 8 : 2)}px` }}
                      />
              ...
              {pendingUsers > 0 && (
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center gap-3">
                  ...
                  <p className="text-sm text-amber-800">
                    <span className="font-bold">{pendingUsers} usuario{pendingUsers !== 1 ? 's' : ''}</span> sin plan activo.
            ...
            <>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 text-sm border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-primary-500"
                />
                {search && (
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                {loading ? (
                  ...
                  <div className="p-12 text-center text-slate-400">
                    <p className="font-semibold mb-2">{search ? 'Sin resultados' : 'Sin usuarios visibles'}</p>
                    {!search && <p className="text-xs">Agrega la política RLS de super admin en Supabase SQL Editor.</p>}
                  </div>
                ) : (
                              ...
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-1 flex-wrap">
                                  <button
                                    ...
                                    disabled={busy}
                                    className="text-xs bg-primary-600 hover:bg-primary-700 disabled:bg-slate-200 text-white font-semibold px-2 py-1.5 rounded-lg transition-colors whitespace-nowrap"
                                  >
                                    {busyId === u.id ? '...' : 'Activar'}
                                  </button>
                                  <button
                                    ...
                                    className="text-xs bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-200 text-white font-semibold px-2 py-1.5 rounded-lg transition-colors flex items-center gap-1"
                                  >
                                    <Gift size={10} />
                                  ...
                                  {isBlocked ? (
                                    <button
                                      ...
                                      className="text-xs bg-blue-100 hover:bg-blue-200 text-blue-700 disabled:opacity-40 font-semibold px-2 py-1.5 rounded-lg transition-colors flex items-center gap-1"
                                    >
                                      <CheckCircle size={10} />
                                    ...
                                    <button
                                      ...
                                      className="text-xs bg-red-100 hover:bg-red-200 text-red-700 disabled:opacity-40 font-semibold px-2 py-1.5 rounded-lg transition-colors flex items-center gap-1"
                                    >
                                      <Ban size={10} />
                                  ...
                                  {!isSelf && (
                                    <button
                                      ...
                                      className="text-xs bg-slate-100 hover:bg-red-100 text-slate-500 hover:text-red-700 disabled:opacity-40 font-semibold px-2 py-1.5 rounded-lg transition-colors flex items-center gap-1"
                                    >
                                      <Trash2 size={10} />
                                      {busyId === u.id + '_del' ? '...' : ''}
                                    </button>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                <p className="text-xs font-semibold text-amber-800 mb-2">
                  Para ver todos los usuarios y sus notas, ejecuta estas políticas RLS en Supabase → SQL Editor:
                </p>
...
create policy "Super admin reads all consultations"
  on public.consultations for select
  using (auth.jwt() ->> 'email' = 'jaimepinedo95@gmail.com' OR auth.uid() = user_id);`}</pre>
              </div>
            </>
          )}
            ...
            <>
              {metricsLoading ? (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {[1, 2, 3, 4].map(i => (
                    <div key={i} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 h-48 animate-pulse" />
                  ))}
                </div>
              ) : (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  {/* Note types — real DB data */}
                  <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                    <h2 className="font-bold text-slate-900 mb-4">Notas por tipo</h2>
                    <div className="space-y-3">
                      {notesByType.map(({ type, count }) => (
                        <MiniBar key={type} label={NOTE_TYPE_LABELS[type] ?? type} count={count} max={maxTypeCount} />
                      ))}
                      {notesByType.length === 0 && <p className="text-xs text-slate-400">Sin datos aún.</p>}
                    </div>
                  </div>

                  {/* Notes by specialty */}
                  <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
                    <h2 className="font-bold text-slate-900 mb-4">Notas por especialidad</h2>
                    <div className="space-y-3">
                      {notesBySpecialty.map(({ specialty, count }) => (
                        <MiniBar key={specialty} label={specialty} count={count} max={maxSpecialtyCount} />
                      ))}
                      {notesBySpecialty.length === 0 && <p className="text-xs text-slate-400">Sin datos aún.</p>}
                    </div>
                  </div>
                  ...
                  {/* Notes by day — 30-day bar chart */}
                  <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 lg:col-span-2">
                    <h2 className="font-bold text-slate-900 mb-4">Notas por día — últimos 30 días</h2>
                    <div className="flex items-end gap-1 h-32">
                      {notesByDay.map(({ day, count }) => {
                        const label = new Date(day + 'T12:00:00').toLocaleDateString('es-CO', { day: 'numeric', month: 'short' })
                        return (
                          <div key={day} className="flex-1 flex flex-col items-center gap-1 group relative">
                            <div
                              className="w-full rounded-t bg-primary-500 group-hover:bg-primary-600 transition-all cursor-default"
                              style={{ height: `${Math.max((count / maxDayNotes) * 110, count > 0 ? 6 : 2)}px` }}
                            />
                            <div className="absolute -top-8 left-1/2 -translate-x-1/2 hidden group-hover:block bg-slate-900 text-white text-xs px-2 py-1 rounded whitespace-nowrap z-10">
                              {label}: {count}
                            </div>
                          </div>
                        )
                    ...
                    <div className="flex justify-between mt-2 text-xs text-slate-400">
                      <span>
                        {notesByDay[0]?.day && new Date(notesByDay[0].day + 'T12:00:00').toLocaleDateString('es-CO', { day: 'numeric', month: 'short' })}
                      </span>
                      <span>Hoy</span>
                    </div>
                  </div>
